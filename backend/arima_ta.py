...
def determine_manual_order(series, max_lag=10):
    from statsmodels.tsa.stattools import acf, pacf

    series = series.dropna()
    n = len(series)

    # Tentukan lag maksimal sesuai aturan PACF/ACF
    safe_lag = min(max_lag, max(1, (n // 2) - 1))

    # Hitung ACF dan PACF
    acf_vals = acf(series, nlags=safe_lag)
    pacf_vals = pacf(series, nlags=safe_lag, method='ywm')

    # Tentukan p
    p = 0
    for lag in range(1, safe_lag + 1):
        if abs(pacf_vals[lag]) < 1.96 / np.sqrt(n):
            break
        p = lag
    ...
    q = 0
    for lag in range(1, safe_lag + 1):
        if abs(acf_vals[lag]) < 1.96 / np.sqrt(n):
            break
        q = lag
...
def main():
    ...
    for prod in products:
        ...
        # -----------------------------------------
        # PENENTUAN MANUAL (HANYA 1 PRODUK)
        # -----------------------------------------
        if prod == products[0]:  # ambil produk pertama saja
            print(f"\n📌 Penentuan Manual ARIMA untuk produk: {prod}")
            p_manual, q_manual = determine_manual_order(df_product[col_used])
            print(f"   → p_manual = {p_manual}, q_manual = {q_manual}, d = {d}")

            # Tampilkan ACF-PACF sebagai bukti manual
            plot_acf_pacf(df_product[col_used], prod)
