...
# ======================================================
# TRAINING + EVALUASI STABIL (ARIMA → ETS → NAIVE)
# ======================================================
def train_stable_model(df_product, product_name, order, col):
    ...
    plt.plot(test.index, forecast, label="Test (Forecast)", color="red", linestyle="--")
    plt.title(f"{product_name} | Stable Forecast")
    plt.legend()
    plt.tight_layout()
    plt.show()
...
def main():
    ...
    for model_row in arima_results:
...
if __name__ == "__main__":
    ...
    if not df_eval.empty:
        for row in df_eval.itertuples():
            if row.status in ("OVERFIT", "UNDERFIT"):  # meski harusnya STABLE semua
